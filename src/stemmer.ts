...
function porterStemmer(w: string) {
	let stem,
		suffix,
		re,
		re2,
	...
	const firstChar = w.substr(0, 1);
	if (firstChar === 'y') {
		w = firstChar.toUpperCase() + w.substr(1);
	}
	...
	if (firstChar === 'y') {
		w = firstChar.toLowerCase() + w.substr(1);
	}
 ...
 * @function
 */
export function stemmer(token: Token) {
	return token.update(porterStemmer);
}

Pipeline.registerFunction(<PipelineFunction> stemmer, 'stemmer');
